...
fn parse_device_role(value: &str) -> DeviceRole {
    match value {
...
fn parse_entry_kind(value: &str) -> EntryKind {
    match value {
...
fn parse_hash_status(value: &str) -> HashStatus {
    match value {
...
fn parse_change_kind(value: &str) -> ChangeKind {
    match value {
...
fn parse_history_reason(value: &str) -> HistoryReason {
    match value {
...
fn sync_task_from_row(row: &Row<'_>) -> rusqlite::Result<SyncTask> {
    ...
    Ok(SyncTask {
        ...
        local_role: parse_device_role(&role_str),
...
fn file_snapshot_from_row(row: &Row<'_>) -> rusqlite::Result<FileSnapshot> {
    let kind: String = row.get(2)?;
    let hash_status: String = row.get(6)?;
    Ok(FileSnapshot {
        ...
        hash_status: parse_hash_status(&hash_status),
        deleted: row.get::<_, i32>(7)? != 0,
        is_symlink: row.get::<_, i32>(8)? != 0,
    })
}

fn sync_baseline_from_row(row: &Row<'_>) -> rusqlite::Result<SyncBaseline> {
    let primary_hash_status: String = row.get(3)?;
    let secondary_hash_status: String = row.get(8)?;
    Ok(SyncBaseline {
        task_id: parse_uuid_text(row.get(0)?, 0)?,
        relative_path: row.get(1)?,
        primary_hash: row.get(2)?,
        primary_hash_status: parse_hash_status(&primary_hash_status),
        primary_size: row.get(4)?,
        secondary_size: row.get(5)?,
        primary_modified_unix_ms: row.get(6)?,
        secondary_hash: row.get(7)?,
        secondary_hash_status: parse_hash_status(&secondary_hash_status),
        secondary_modified_unix_ms: row.get(9)?,
        last_synced_unix_ms: row.get(10)?,
    })
}

fn pending_return_change_from_row(row: &Row<'_>) -> rusqlite::Result<PendingReturnChange> {
    let change_kind: String = row.get(2)?;
    let secondary_hash_status: String = row.get(4)?;
...
fn history_entry_from_row(row: &Row<'_>) -> rusqlite::Result<HistoryEntry> {
    ...
    Ok(HistoryEntry {
        ...
        reason: parse_history_reason(&reason),
        created_unix_ms: row.get(5)?,
        size: row.get(6)?,
    })
}

fn deferred_transfer_from_row(row: &Row<'_>) -> rusqlite::Result<DeferredTransferRecord> {
...
fn paired_device_from_row(row: &Row<'_>) -> rusqlite::Result<PairedDevice> {
    Ok(PairedDevice {
        device_id: row.get(0)?,
...
impl<'a> FileSnapshotRepository<'a> {
    ...
    pub fn list_by_task(&self, task_id: &Uuid) -> Result<Vec<FileSnapshot>> {
        ...
        let rows = stmt.query_map(params![task_id.to_string()], file_snapshot_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
...
impl<'a> SyncBaselineRepository<'a> {
    ...
    pub fn list_by_task(&self, task_id: &Uuid) -> Result<Vec<SyncBaseline>> {
        ...
        let rows = stmt.query_map(params![task_id.to_string()], sync_baseline_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
...
impl<'a> PendingReturnRepository<'a> {
    ...
    pub fn list_by_task(&self, task_id: &Uuid) -> Result<Vec<PendingReturnChange>> {
        ...
        let rows = stmt.query_map(params![task_id.to_string()], pending_return_change_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
    ...
    pub fn get(&self, task_id: &Uuid, relative_path: &str) -> Result<Option<PendingReturnChange>> {
        ...
        let result = stmt.query_row(
            params![task_id.to_string(), relative_path],
            pending_return_change_from_row,
        );
        match result {
...
impl<'a> HistoryRepository<'a> {
    ...
    pub fn list_by_task(&self, task_id: &Uuid) -> Result<Vec<HistoryEntry>> {
        ...
        let rows = stmt.query_map(params![task_id.to_string()], history_entry_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
...
impl<'a> DeferredTransferRepository<'a> {
    ...
    pub fn list_all(&self) -> Result<Vec<DeferredTransferRecord>> {
        ...
        let rows = stmt.query_map([], deferred_transfer_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
...
impl<'a> PairedDeviceRepository<'a> {
    ...
    pub fn get(&self, device_id: &str) -> Result<Option<PairedDevice>> {
        ...
        let result = stmt.query_row(params![device_id], paired_device_from_row);
        match result {
            Ok(d) => Ok(Some(d)),
            Err(rusqlite::Error::QueryReturnedNoRows) => Ok(None),
            Err(e) => Err(e.into()),
    ...
    pub fn list_all(&self) -> Result<Vec<PairedDevice>> {
        ...
        let rows = stmt.query_map([], paired_device_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
...
impl<'a> LogRepository<'a> {
    ...
    pub fn list_recent(&self, limit: usize) -> Result<Vec<LogEntry>> {
        ...
        let rows = stmt.query_map(params![limit as i64], log_entry_from_row)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
