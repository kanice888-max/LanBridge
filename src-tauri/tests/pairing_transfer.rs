...
use lanbridge::transport::{decode_message, encode_message, ConnectionManager, SyncMessage};
...
fn test_connecting_discovered_peer_pins_real_device_id() {
    ...
    let device_id = pin_connected_peer(&manager, "192.168.1.20", 9527, Some(peer));
...
async fn test_full_discovery_pair_task_plan_transfer_ack_and_status_flow() {
    ...
    let platform = WinPlatform::with_data_dir(primary_dir.path().join("app-data"));
